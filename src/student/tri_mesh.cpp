...
Trace Triangle::hit(const Ray& ray) const {
    ...
    Vec3 s = ret.origin - v_0.position;
    Vec3 d = ray.dir;
    //parallel
    ...
    auto u = -1 * dot(cross(s, e2), d) / dot(cross(e1, d), e2);
    auto v = dot(cross(e1, d), s) / dot(cross(e1, d), e2);
    auto t = -1 * dot(cross(s, e2), e1) / dot(cross(e1, d), e2);
    if(u < 0 || v < 0 || u + v > 1 || t > ray.dist_bounds.y || t < ray.dist_bounds.x) {
        ret.hit = false;       // was there an intersection?
        ret.distance = 0.0f;   // at what distance did the intersection occur?
        ret.position = Vec3{}; // where was the intersection?
        ret.normal = Vec3{};   // what was the surface normal at the intersection?
